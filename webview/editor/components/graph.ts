...
export class Graph {
    ...
    private _findSubtreeRoot(id: string): G6NodeData | null {
        ...
        if (data?.path) {
            ...
            return (
                this._getAncestors(id).find((v) => (v.data as unknown as NodeData)?.path) ?? null
            );
        }
    }
    ...
    clickVar(...names: string[]) {
        ...
        for (const [node, states] of nodes) {
            ...
            const newStates = [
                ...oldStates.filter((v) => !this._isHighlightState(v)),
                ...states,
            ].sort();
            if (
                oldStates.length !== newStates.length ||
                oldStates.some((v, i) => v !== newStates[i])
            ) {
