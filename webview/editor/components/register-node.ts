...
const calcTextWith = (text: string, fontSize?: string) => {
    ...
    if (!fontSize) {
        b3Workspace = document.querySelector(".b3-editor");
        if (b3Workspace) {
...
const hasErrorInArgExpr = (def: NodeDef, data: NodeData) => {
    const checkExpr = useWorkspace.getState().checkExpr;
    ...
    render(attributes?: Required<RectStyleProps> | undefined, container?: Group): void {
        ...
        if (
            ...
            ((nodeDef as NodeDef & { group?: string[] }).group?.length &&
                !(nodeDef as NodeDef & { group?: string[] }).group!.some(
                    (g) => b3util.usingGroups?.[g]
                )) ||
