...
const hotkeyMap: Record<string, "copy" | "paste" | "replace" | "insert" | "delete" | "undo" | "redo"> =
  {
    [Hotkey.Copy]: "copy",
...
export const GraphPane: React.FC = () => {
  ...
  const canOpenSubtree = Boolean(selectedNode?.data.path || (selectedNodeRef?.subtreeStack.length ?? 0) > 0);
  ...
  return (
    <Dropdown menu={{ items: menuItems, onClick: ({ key }) => void runMenuCommand(String(key)) }} trigger={["contextMenu"]}>
