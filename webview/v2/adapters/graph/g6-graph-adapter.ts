...
export class G6GraphAdapter implements GraphAdapter {
  ...
  private updateDragIntent(targetKey: string | null, position: DropIntent["position"] | null) {
    if (
      this.dragIntent.targetKey === targetKey &&
      this.dragIntent.position === position
    ) {
      return;
    }
    ...
    const graph = new G6Graph({
      ...
      layout: {
        ...
        getHeight: (datum: G6NodeData) =>
          Number(
            (datum.data as { height?: number } | undefined)?.height ?? G6_VECTOR_NODE_MIN_HEIGHT
          ),
        getWidth: (datum: G6NodeData) =>
          Number((datum.data as { width?: number } | undefined)?.width ?? G6_VECTOR_NODE_WIDTH),
