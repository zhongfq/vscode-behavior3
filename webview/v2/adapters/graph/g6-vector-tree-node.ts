...
export const VectorTreeNodeStyle: { [s in VectorTreeNodeState]?: { [n in ShapeName]?: NodeStyle } } =
  {
    dragsrc: {
...
class VectorTreeNode extends Rect {
  ...
  private drawWarningText(container: Group) {
    const text = isWarningNode(this.node) ? cutWordTo(`Type:${this.node.typeLabel}`, CONTENT_WIDTH) : "";
