...
- graph 节点卡片不展示字段级校验错误文本
- graph 可以用通用错误态表达“该节点当前无效”，但不在卡片正文里展开字段级报错
- graph 卡片正文文本区只展示 resolved graph 解析级错误
  例如 missing subtree、invalid subtree、cyclic subtree
- 变量未定义、group 未启用、表达式非法、必填缺失等校验说明应留在 Inspector / 编辑提交流程中
...
- full render 前后应尽量保持原视口
- 允许改变视口的只有显式导航型交互
  例如搜索结果聚焦、用户手动拖动画布、用户主动缩放
...
- focus node
- viewport restore
- 宿主变量声明、import/subtree decl 元数据变化但不影响节点几何与解析结果时

要求：

- selection、highlight、Inspector 提交、宿主元数据回流不应隐式推动视口
- graph 容器 resize 后应恢复到 resize 前的视口
